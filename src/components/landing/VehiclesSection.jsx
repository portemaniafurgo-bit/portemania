...
export default function VehiclesSection() {
  ...
  const handleContratar = (vehicleKey = null) => {
    if (isAuthenticated) {
      router.push(vehicleKey ? `/new-request?vehicle=${vehicleKey}` : "/new-request");
    } else {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl font-display font-bold text-foreground mt-2">
           ...
           </h3> <p className="text-muted-foreground mt-3 max-w-lg mx-auto">
            Ideal para portes y pequeñas mudanzas en Albacete.
            Donde necesitas una furgoneta, ahí está ClicyVoy. </h3>
          </p>
        </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                <Link href={`/login-clientes${selectedVehicle ? `?vehicle=${selectedVehicle}` : ""}`} onClick={() => setShowModal(false)}>
                ...
                <Link href={`/register${selectedVehicle ? `?vehicle=${selectedVehicle}` : ""}`} onClick={() => setShowModal(false)}>
                ...
                <Link href={`/solicitar${selectedVehicle ? `?vehicle=${selectedVehicle}` : ""}`} onClick={() => setShowModal(false)}>
