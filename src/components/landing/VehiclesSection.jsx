...
export default function VehiclesSection() {
  ...
  const handleContratar = (vehicleKey = null) => {
    const draft = vehicleKey ? { vehicle: vehicleKey } : {};
    if (isAuthenticated) {
      router.push(buildRequestHref("/new-request", draft));
    } else {
  ...
  const selectedDraft = selectedVehicle ? { vehicle: selectedVehicle } : {};
  ...
  return (
    <section className="py-24 bg-muted/50">
      ...
      <AccessModal
        ...
        loginHref={buildRequestHref("/login-clientes", selectedDraft)}
        registerHref={buildRequestHref("/register", selectedDraft)}
        guestHref={buildRequestHref("/solicitar", selectedDraft)}
      />
    </section>
  );
}
