...
## 7. 2026-09-22 — Preparación para Play Store

Toda la parte de código para publicar la app Android, hecha de una vez. Queda fuera lo que no depende del repo (cuentas y credenciales).

- **Versión 1.0.0** en `mobile/app.json` y `mobile/package.json`. Permisos de Android limpios: cada uno UNA vez y `RECORD_AUDIO` fuera de `permissions` (se queda solo en `blockedPermissions`, que es lo que hace que no aparezca en el manifiesto y no haya que justificarlo ante Play).
- **`mobile/app.config.js` nuevo**: capa dinámica sobre `app.json` que engancha el `google-services.json` de Firebase, venga como fichero local o como variable de entorno de tipo fichero de EAS (`GOOGLE_SERVICES_JSON`). Sin él, la build salía sin push y sin decir por qué. `mobile/.gitignore` bloquea ya credenciales (Firebase, Play, keystores) y `eas.json` tiene el `submit.production` apuntando a la pista interna en borrador.
- **Sonido propio del aviso de oferta**: `scripts/generate-notification-sound.mjs` genera `mobile/assets/sounds/oferta.wav` (WAV PCM sin dependencias, 1,4 s, dos golpes de dos notas) y los canales de Android llevan sonido, vibración larga, luces y visibilidad en la pantalla de bloqueo. Gotcha documentado en el código: **Android congela la configuración del canal en la primera instalación**, así que en un móvil que ya tenga la app hay que desinstalarla para oír el sonido nuevo.
- **Divulgación destacada de ubicación en segundo plano** (`mobile/lib/locationDisclosure.js`): Google Play rechaza la app si el permiso de segundo plano sale sin un aviso propio antes que explique qué se recoge, para qué y que ocurre con la app cerrada. Se muestra una vez por instalación desde el trabajo activo del conductor; si dice que no, no se pide el permiso y la pantalla lo explica. `Dialog.js` ganó un `onDismiss` para que cerrar por el fondo cuente como "no" en vez de dejar la promesa colgada.
- **Web**: página pública `/eliminar-cuenta` (Play la exige con URL accesible sin sesión), enlazada desde la política de privacidad y en el sitemap. La web NO tiene borrado automático: quien no use la app lo pide por correo, y así está escrito.
- **Ficha de Play**: `mobile/store/` con el icono 512, el gráfico de cabecera 1024x500, los textos con su recuento de caracteres (`listing.es.md`) y la lista de capturas que hay que hacer en un móvil real con `adb`. Los gráficos salen del mismo logo que la app (`scripts/generate-store-assets.mjs` importa los trazados de `generate-app-assets.mjs`, ahora refactorizado para exportarlos sin escribir nada al importarlo).
- **Documentación**: `docs/PLAY-STORE.md` (runbook completo: Firebase, firma, build, primera subida a mano, TODOS los formularios de Play con las respuestas propuestas, Stripe live, OTA) y `docs/BRIEF-CLAUDE-CHROME-PLAY.md` (tareas clic a clic para el agente del navegador).

**Lo que queda, y no es código**: cuenta de Play Console (25 USD; decidir personal —12 testers durante 14 días— u organización con DUNS), proyecto de Firebase, publicar la consent screen de Google, decidir el paso de Stripe a claves live, las capturas y el vídeo del permiso de ubicación, y crear/borrar las cuentas de prueba para los revisores.
