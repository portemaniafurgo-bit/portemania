...
function estimatePrice(vehicleType, extraHours, insurance) {
  ...
  return base + (extraHours || 0) * EXTRA_HOUR_PRICE + (insurance ? INSURANCE_PRICE : 0);
}

export default function GuestRequest() {
  const navigate = useNavigate();
  const preselectedVehicle = new URLSearchParams(window.location.search).get("vehicle") || "";
  ...
  // Map logical steps: if vehicle preselected, step 3 (vehicle) is skipped
  // Steps: 1=addresses, 2=cargo, 3=vehicle(if needed), 4=review → shown as 3 when skipped
  const nextStep = () => {
    if (step === 1) simulateDistance();
    const next = step + 1;
    // Skip vehicle step (3) if preselected
    ...
    <div className="max-w-xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex gap-1.5">
        {Array.from({ length: totalSteps }).map((_, i) => {
          ...
          // Map visual step to actual step when vehicle is preselected
          ...
          <motion.div key="step2" variants={stepVariants} initial="initial" animate="animate" exit="exit" className="space-y-5">
            ...
            <div className="space-y-3">
              <button type="button" onClick={() => setAcceptPortal(v => !v)} className="flex items-start gap-3 w-full text-left">
                ...
                <span className="text-sm text-foreground">Acepto que la mercancía sea recogida en el portal <span className="text-destructive">*</span></span>
              </button>
            ...
            {form.vehicle_type && (
              <div className="bg-card rounded-2xl border border-border p-5 space-y-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Cada hora adicional cuesta {EXTRA_HOUR_PRICE}€.</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">{form.extra_hours === 0 ? "Solo 2h incluidas" : `+${form.extra_hours}h extra (+${form.extra_hours * EXTRA_HOUR_PRICE}€)`}</p>
                  </div>
